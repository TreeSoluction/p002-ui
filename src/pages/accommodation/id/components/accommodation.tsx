...
export function AccommodationView({ accommodation }: AccommodationViewProps) {
  const [selectedImage, setSelectedImage] = useState<string | null>(null);

  const hasInstagram =
    !!accommodation.instagram?.trim() &&
    accommodation.instagram.trim().toUpperCase() !== "SEM_INSTAGRAM";

  const hasWhatsapp =
    !!accommodation.whatsapp?.trim() &&
    accommodation.whatsapp.trim().toUpperCase() !== "SEM_WHATSAPP";

  const instagramUsername = accommodation.instagram
    ...
    <>
      <div className="w-full bg-white rounded-xl shadow-md overflow-hidden">
        <div className="flex flex-col sm:flex-row gap-4 p-4 bg-gray-100 rounded-lg">
          ...
          <div className="text-sm text-gray-800 whitespace-pre-line wrap-break-word leading-relaxed sm:mt-1">
            {accommodation.local}
          </div>
        </div>
        ...
        <div className="flex gap-4 justify-center sm:justify-start p-4 pt-2 flex-wrap">
          {hasInstagram && instagramUsername && (
            <Link
              to={`https://instagram.com/${instagramUsername}`}
              target="_blank"
          ...
          {hasWhatsapp && (
            <Link
              to={`https://wa.me/${accommodation.whatsapp.replace(/\D/g, "")}?text=Olá%2C%20vim%20do%20Feiras%20de%20Pernambuco%2C%20gostaria%20de%20mais%20informações.`}
              target="_blank"
              rel="noopener noreferrer"
              className="w-[140px] flex flex-col items-center bg-gray-200 rounded-md px-2 py-1 hover:bg-gray-300"
            >
        ...
        <div className="flex flex-wrap gap-2 p-4 justify-center">
          {accommodation.produtos && accommodation.produtos.length > 0 ? (
            accommodation.produtos.map((base64, index) => (
              <img
                key={index}
                src={base64}
                alt={`Produto ${index + 1}`}
                className="rounded-md object-cover cursor-pointer shrink-0 w-[48%] sm:w-[30%] md:w-[23%]"
